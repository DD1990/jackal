...
func (s *inStream) handleSessionStarted(elem xmpp.XElement) {
	...
	//start - update - lxf - 20181025 TODO(lxf) 会导致组件无法发送给客户端，如果不加，消息发送模块需要拦截客户端是否在同一个虚拟域
	/*if stanza.ToJID().Domain() != stanza.FromJID().Domain() {
				failure := xmpp.NewElementNamespace("failure", "")
				failure.AppendElement(xmpp.NewElementName(""))
				s.writeElement(failure)
		return
	}*/
	//end - update - lxf - 20181025
	if comp := component.Get(stanza.ToJID().Domain()); comp != nil { // component stanza?
		comp.ProcessStanza(stanza, s)
	} else if comp := commponentInter.Get(stanza.ToJID().Domain()); comp != nil { // component stanza?
		comp.ProcessStanza(stanza, s)
	} else {
...
func (s *inStream) proceedStartTLS(elem xmpp.XElement) {
	...
	//cfg.Certificates[0],cfg.Certificates[1] = cfg.Certificates[1],cfg.Certificates[0]
	domtls := &tls.Config{Certificates: host.Certificates()}
	for _, hname := range host.HostNames() {
		if hname == s.Domain() {
			domtls.Certificates[0] = host.Hosts()[hname]
		}
	}
	s.cfg.transport.StartTLS(domtls, false)
