...
func main() {
	...
	flag.StringVar(&configFile, "config", "./example.jackal.yml", "Configuration file path.")
	flag.StringVar(&configFile, "c", "./example.jackal.yml", "Configuration file path.")
	flag.Usage = func() {
		for i := range logoStr {
	...
	host.Initialize(cfg.Hosts) //放在 初始化模块及组件 之后，host加载数据库动态虚拟域用到 HTTP-API组件的持久层
